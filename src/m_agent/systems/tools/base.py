"""Shared dataclasses used by the tool-suite system.

* :class:`ControllerCapabilitySpec` — declarative spec for one capability
  (name + tool-factory callable).
* :class:`ControllerCapabilityContext` — the per-execution mutable
  context that the spec's ``build_tool`` closes over.

The context carries an ``episodic_backend`` field so capability
implementations (e.g. ``recall.py``) call into the episodic-memory
plug-in (:class:`~m_agent.systems.episodic.protocols.EpisodicMemoryBackend`).
"""
...
class ControllerCapabilityContext:
    ...
    #: Episodic-memory backend (:class:`EpisodicMemoryBackend` protocol).
    #: Capability implementations (e.g. recall) must use
    #: :meth:`get_episodic_backend` to reach this backend.
    ...
    def get_episodic_backend(self) -> Any:
        """Return the configured episodic backend (required for recall tools)."""
        if self.episodic_backend is not None:
            return self.episodic_backend
        raise RuntimeError(
            "Episodic recall is unavailable because episodic_backend is not configured."
        )
