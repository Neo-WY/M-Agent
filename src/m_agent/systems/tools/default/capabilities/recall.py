"""Recall capabilities backed by the episodic-memory plug-in.

The two LangChain tools defined here (``shallow_recall`` / ``deep_recall``)
...
def _build_shallow_recall_tool(context: ControllerCapabilityContext, description: str):
    ...
    def shallow_recall(question: str) -> Dict[str, Any]:
        """Single-round episodic recall through the configured backend."""
        ...
        backend = context.get_episodic_backend()
        try:
...
def _build_deep_recall_tool(context: ControllerCapabilityContext, description: str):
    ...
    def deep_recall(question: str) -> Dict[str, Any]:
        """Multi-round episodic recall through the configured backend."""
